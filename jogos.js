...
{ // INÍCIO DO BLOCO PARA EVITAR CONFLITOS DE CONSTANTES E FUNÇÕES
  ...
  function renderStatusMercado(mercado) {
    ...
    return `
      <div class="bg-white rounded-2xl shadow-sm border mx-4 mb-4">
        <div class="bg-orange-50 px-4 py-2.5 border-b">
          <p class="text-xl font-black text-center">Jogos da Rodada</p>
        </div>
  ...
  async function buscarPartidas(rodada) {
    ...
    let url;
    if (isCurrent) {
      url = API_CARTOLA_LOCAL.PARTIDAS;
    } else {
      url = API_CARTOLA_LOCAL.PARTIDAS_RODADA(rodada);
    }
  ...
  async function buscarPontuados(rodada) {
    ...
    const proxyUrl = API_CARTOLA_LOCAL.PONTUADOS(rodada);
    try {
  ...
  async function buscarValorizacao() {
    try {
      const rota = API_CARTOLA_LOCAL.AWS_ATLETAS_PONTUADOS;
  ...
  async function abrirModalScouts(partida) {
    ...
    const renderizarLista = (timeId) => {
      ...
      if (atletasTime.length === 0) {
        body.innerHTML = `<div class="empty-scouts text-center py-4 text-gray-400 text-xs">NENHUM ATLETA EM CAMPO</div>`;
        return;
      }
      body.innerHTML = atletasTime.map(atleta => {
        const sigla = siglaPosicao[atleta.posicao_id] || "???";
        const scoutsList = Object.entries(atleta.scout || {}).map(([k,v]) => `<span class="scout-item bg-gray-50 px-1 rounded text-[9px] text-gray-500 font-bold">${v}${k.toUpperCase()}</span>`).join(" ");
        let emojis = [];
        ...
        const emojiSpan = emojis.length ? `<span class="scout-emojis ml-1 text-xs">${emojis.join(" ")}</span>` : "";
        ...
        return `
          <div class="atleta-card flex items-center justify-between p-3 border-b border-gray-50 last:border-0">
            <div class="atleta-info flex items-center gap-3">
              <img src="${atleta.foto?.replace("FORMATO", "140x140") || ""}" class="w-10 h-10 rounded-full bg-gray-100" onerror="this.src='./images/default.png'">
              <div class="atleta-dados">
                <div class="atleta-posicao text-[8px] font-bold text-gray-400 uppercase">${sigla}</div>
                <div class="atleta-nome text-sm font-black text-gray-800">${atleta.apelido}</div>
                ${valHtml}
              </div>
            </div>
            <div class="atleta-stats text-right">
              <div class="pontuacao-wrapper flex items-center justify-end">
                <span class="pontuacao font-black text-lg ${pontuacaoClass}">${pontuacao}</span>${emojiSpan}
              </div>
              <div class="scouts-wrapper flex flex-wrap justify-end gap-1 mt-1 max-w-[120px] ml-auto">${scoutsList || '<span class="text-gray-300 text-[10px]">—</span>'}</div>
            </div>
          </div>
        `;
      }).join("");
    ...
    const modalHtml = `
      <div id="modal-scouts" class="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm" onclick="if(event.target === this) window.fecharModalScouts()">
        <div class="relative w-full max-w-md mx-3 bg-white rounded-2xl shadow-2xl overflow-y-auto max-h-[90vh]">
          <div class="sticky top-0 bg-white z-10 border-b border-gray-100 px-4 py-3 flex justify-between items-center">
            ...
            <button onclick="window.fecharModalScouts()" class="w-7 h-7 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center text-gray-600">
          ...
          <div class="modal-body p-2 space-y-0"></div>
        </div>
      </div>
    `;
  ...
  function renderCardPartida(p, rodadaCard, rodadaAtual) {
    ...
    return `
      <div class="match-card-v2 bg-white rounded-2xl shadow-sm border p-4 mb-4 cursor-pointer" data-partida-id="${p.partida_id}" data-rodada="${rodadaCard}">
        <p class="text-[10px] text-gray-400 text-center mb-2">${formatarData(p.partida_data)} • ${p.local || "-"}</p>
        <div class="flex items-start justify-between gap-2">
          <div class="flex-1 text-center">
            <span class="text-[11px] text-gray-400 font-bold">${formatarPosicao(p.clube_casa_posicao)}</span>
          ...
          <div class="flex-1 text-center">
            <span class="text-[11px] text-gray-400 font-bold">${formatarPosicao(p.clube_visitante_posicao)}</span>
        ...
        <button class="expand-top5-btn w-full mt-3 py-1 bg-gray-50 rounded-lg text-[9px] font-black uppercase text-gray-400 hover:text-orange-500 transition">▼ MOSTRAR TOP 5</button>
      </div>
    `;
  }
  ...
  function gerarTop5Html(partida) {
    ...
    const renderLista = (lista) => {
      ...
      return lista.map(a => `
        <div class="flex justify-between items-center py-1 border-b border-gray-50 last:border-0">
          <span class="text-[11px] font-bold text-gray-600 truncate max-w-[80px]">${a.apelido}</span>
    ...
    return `
      <div class="grid grid-cols-2 gap-4">
        <div><p class="text-center font-black text-[10px] uppercase mb-2 text-gray-400">${casaNome}</p>${renderLista(atletasCasa)}</div>
        <div><p class="text-center font-black text-[10px] uppercase mb-2 text-gray-400">${foraNome}</p>${renderLista(atletasFora)}</div>
      </div>
    `;
  }
  ...
  window.renderJogos = async function() {
    ...
    try {
      const resMerc = await fetch(API_CARTOLA_LOCAL.MERCADO_STATUS);
  ...
  console.log("✅ jogos.js sincronizado e protegido (IIFE)");
}
